...
1. **Search existing codebase first**: `search_file_content --pattern "function_name"`
...
2. **Run `uv run pre-commit run --all-files`** - must pass completely
...
2. **Run `uv run pre-commit run --all-files`** - All quality gates must pass completely
3. **Search existing code first** - Use `search_file_content` to find similar functionality before implementing
...
```
1. Is this core business logic for the d3_item_salvager project?
...
4. Always check these common locations:
   - src/d3_item_salvager/config.py (configuration utilities)
   - src/d3_item_salvager/services.py (service patterns)
   - src/d3_item_salvager/exceptions.py (error types)
```
...
1. **Search the entire codebase** for similar functionality:
   ...
   - Search for similar parameter types (`timeout: int`, `config: d3_item_salvager.Config`)
...
search_filecontent "def validate" src/
search_filecontent "ValueError.*positive" src/
search_filecontent "timeout.*int" src/

# Before creating a new service, check existing patterns:
search_filecontent "class.*Service" src/
search_filecontent "__init__.*config.*runtime" src/
```
...
4. **Check**: Run `uv run pre-commit run --all-files` to ensure quality
...
```bash
uv run pre-commit run --all-files  # Must pass all tests and quality checks
```
...
```python
from d3_item_salvager.config import d3_item_salvager.Config, RuntimeOptions

# Immutable static configuration
config = d3_item_salvager.Config.from_env()  # Load from environment
...
class MyService:
    def __init__(self, config: d3_item_salvager.Config, runtime: RuntimeOptions):
        self.config = config
...
1. **Google-style docstrings** on all public functions, methods, and classes:
   ...
   ```python
   def process_data(items: List[str], config: d3_item_salvager.Config) -> Dict[str, Any]:
...
```bash
uv run pre-commit run --all-files              # Standard workflow
```

**Before PR (MANDATORY):**

1. Run `uv run pre-commit run --all-files` - must pass completely
...
- **Test-Accompanying Development:** Every new or modified source file in `src/` must have a corresponding test file in `tests/`, with a mirrored directory structure.
- **Single Source of Truth:** To prevent project clutter and context pollution, all changes should be made in-place, and the repository must not contain duplicate or temporary files.
  - **Exception for Complex Refactoring:** For a complex refactoring task where a temporary reference file is necessary, the following strict, self-cleaning workflow is required:
    1. **Use a Safe Name:** Name the file with a specific, non-source extension (e.g., `my_module.py.refactor_backup`) so it can be easily identified and ignored during other operations.
    2. **Guarantee Cleanup:** The temporary file **must** be deleted as the very next step upon completion or cancellation of the refactoring task. The workspace must be left clean.

### Test Structure & Naming

- **File Naming:** Test files must be named `test_*.py`.
- **Function Naming:** Test functions must start with `test_` and have descriptive names that clearly state the behavior being tested. Use underscores to separate logical parts: `<unit_under_test>_<condition>_<expected_result>`.
  - **Example:** `test_user_service_with_invalid_email_raises_validation_error()`
- **Class Naming:** If using test classes, prefix with `Test` and use PascalCase (e.g., `class TestMyFeature:`).
- **Directory Structure:** The structure under `tests/` must mirror ` /` for easy navigation. Place integration and E2E tests in dedicated subfolders if needed.

### Project Directory Layout

Use the **src layout** for all projects. This structure is designed to scale and accommodate various components like web interfaces, database models, and background workers.
...
│       ├── config.py               # Configuration classes (e.g., Pydantic)
│       ├── exceptions.py           # Custom exception classes
│       ├── core/                   # Core business logic and domain models
│       │   ├── __init__.py
│       │   ├── models.py           # Core domain models (e.g., User, Order)
│       │   └── services.py         # Business logic entry points (e.g., UserService)
│       ├── data/                   # Data access layer
│       │   ├── __init__.py
│       │   ├── repositories.py     # Data access abstractions (e.g., UserRepository)
│       │   └── db.py               # Database session management (e.g., SQLAlchemy)
│       ├── web/                    # Web interface (e.g., FastAPI, HTMX)
│       │   ├── __init__.py
...
│       │       └── index.html
│       ├── workers/                # Background tasks (e.g., Celery, Dramatiq)
│       │   ├── __init__.py
│       │   └── tasks.py
...
│   │   └── core/
│   │       └── test_services.py
│   ├── integration/              # Tests for component interactions (e.g., service + db)
│   │   └── data/
│   │       └── test_repositories.py
│   ├── e2e/                      # End-to-end tests simulating user workflows
│   │   └── web/
│   │       └── test_user_journey.py
│   └── fakes/                      # Reusable test doubles (e.g., fake repository)
│       ├── fake_repository.py
│       └── fake_email_client.py
│
├── scripts/
│   └── check                     # Script for running all quality checks
...
from d3_item_salvager.services.user_service import UserService
from d3_item_salvager.models.user import User
```

**✅ Re-export clean interfaces in `__init__.py` if needed:**

```python
# src/d3_item_salvager/__init__.py
from .services import UserService, OrderService
from .models import User, Order
from .exceptions import d3_item_salvager.Error, ConfigurationError

__all__ = ['UserService', 'OrderService', 'User', 'Order', 'd3_item_salvager.Error', 'ConfigurationError']
```
...
uv run ruff format .       # Format code
uv run pyright            # Type check
uv run python -m d3_item_salvager     # Run application
```
...
# Immutable static configuration
config = d3_item_salvager.Config.from_env()
